...
import { errorToast, showToast } from '@components/Ui/CustomToast';
...
const AddReturnPhotos: React.FC<Props> = ({
  ...
  const handleAddPhoto = async () => {
    ...
    if (imageUri.length >= 3) {
      showToast('error','Alert!', 'Maximum 3 images allowed');
    ...
    if (result?.assets && result.assets.length > 0) {
      ...
      if (file.uri) {
        ...
        errorToast('File URI is undefined');
  ...
  const handleSubmit = async () => {
    if (imageUri.length === 0) {
      showToast('error','Alert!', 'Please add at least one photo');
    ...
    try {
      ...
      errorToast('An error occurred while uploading photos.');
    } finally {
      setLoading(false);
    }
  };
